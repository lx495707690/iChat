...
public class NumberPicker extends LinearLayout implements Button.OnClickListener,
    ...
    private NumberPickerDialogFragment mPickerDialogFrag;
    ...
    public NumberPickerDialogFragment getmPickerDialogFrag() {
		return mPickerDialogFrag;
	}

	public void setmPickerDialogFrag(NumberPickerDialogFragment mPickerDialogFrag) {
		this.mPickerDialogFrag = mPickerDialogFrag;
	}
    ...
    protected void onFinishInflate() {
        ...
        //random digits
        //shuffleArray(numbArray);
        
        for (int i = 0; i < 10; i++) {
            mNumbers[i].setOnClickListener(this);
            mNumbers[i].setText(String.format("%d", numbArray[i]));
            //mNumbers[i].setText(String.format("%d", i));
            //mNumbers[i].setTag(R.id.numbers_key, new Integer(i));
            mNumbers[i].setTag(R.id.numbers_key, numbArray[i]);
        }
        updateNumber();
       ...
       // mLabel = (TextView) findViewById(R.id.label);
        mLabel = (TextView) findViewById(R.id.textViewCustom);
        mSign = SIGN_POSITIVE;
    ...
    protected void doOnClick(View v) {
        ...
        if (val != null) {
           ...
           // mPickerDialogFrag.imitateSetPressed();
        } else if (v == mDelete) {
            if (mInputPointer >= 0) {
    ...
    protected void updateNumber() {
        ...
        if( numberString.length()>6)return; //max 6 number
...
//            } else {
    ...
    public double getEnteredNumberDouble() {
        String value = "0";
        for (int i = mInputPointer; i >= 0; i--) {
            if (mInput[i] == -1) {
                break;
    ...
    private void enableSetButton() {
        ...
        if(mInputPointer >= 5){
        mSetButton.setEnabled(true);//pin number max
        //getmPickerDialogFrag().imitateSetPressed();
        }
    }
    ...
    public int getNumber() {
        String numberString = getEnteredNumber();//Double.toString(getEnteredNumber());
        String[] split = numberString.split("\\.");
    ...
    public double getDecimal() {
        double decimal = BigDecimal.valueOf(getEnteredNumberDouble()).divideAndRemainder(BigDecimal.ONE)[1].doubleValue();
        return decimal;
    }
