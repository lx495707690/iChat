...
public class FragmentContacts extends GenericFragmentiChat {
	...
	private void filledData() {
		...
		Collections.sort(mSourceDataList, new MixComparator());
	}
	...
	public class MixComparator implements Comparator<BeanContact>
	{
		public int compare(BeanContact contact1, BeanContact contact2)
		{
			String o1 = Helper.converterToPinYin(contact1.getName());
			String o2 = Helper.converterToPinYin(contact2.getName());
			// 判断是否为空""
			if (Helper.isEmpty(o1) && Helper.isEmpty(o2))
				return 0;
			if (Helper.isEmpty(o1))
				return -1;
			if (Helper.isEmpty(o2))
				return 1;
			String str1 = "";
			String str2 = "";
			try {
			...
			if (Helper.isWord(str1) && Helper.isWord(str2)) {               //字母
				return str1.compareTo(str2);
			}
			else if (Helper.isNumeric(str1) && Helper.isWord(str2)) {     //数字字母
				return 1;
			}
			else if (Helper.isNumeric(str2) && Helper.isWord(str1)) {
				return -1;
			}
			else if (Helper.isNumeric(str1) && Helper.isNumeric(str2)) {       //数字数字
			...
			else if (Helper.isAllWord(str1) && (!Helper.isAllWord(str2))) {      //数字字母  其他字符
				return -1;
			}
			else if ((!Helper.isAllWord(str1)) && Helper.isWord(str2)) {
				return 1;
			}
			else {
				return 1;
